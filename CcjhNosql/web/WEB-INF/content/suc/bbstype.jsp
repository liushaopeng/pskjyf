...
function del(id) {
	if (confirm('确实要删除吗?')) {
		location.href = "${contextPath}/suc/bbstype!delete.action?_id="
				+ id;
	}
}
function add(){
...
function upd(id){
  ...
  $.post('${ctx}/suc/bbstype!upd.action', submitData, function(json) {
    $('#_id').val(json._id);
        ...
        <div class="input-group ">
            <div style="border-radius:3px; height:40px;padding-left:10px;" class="btn-primary">
                <a href="javascript:add();"style="color: #ffffff;line-height:25px;">
                    分类添加&nbsp;<i class="fa fa-plus"style="line-height:30px;font-size: 14px;"></i>
                </a>
            </div>
         </div>
      </div>
		...
		<div class="col-md-12">
            <div class="table-responsive">
                <table class="table table-striped table-action table-primary mb30" >
                    ...
                    <tbody>
                      <c:forEach items="${bbstypeList}" var="bean">
                      <tr>
                      	<td>${bean.sort}</td>
						<td>${bean.title}</td> 
            ...
            <div class=" bg-bai border-radius3 overflow-hidden">
                <div class="overflow-hidden line-height40 bg-bai line-bottom">
                    <div class="hang50 pull-left zi-hui-tq">
                        <i class="weight500 size14 pl-10 line-height50">论坛分类添加</i>
                    </div>
                ...
                <form id="inscxForm" action="${ctx }/suc/bbstype!save.action?fypage=${fypage}"
